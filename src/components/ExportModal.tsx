...
const ExportModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const startExport = async () => {
    ...
    // Filter characters that actually have dialogues
    ...
    // Canvas height matches exactly the number of bands
    const overlayHeight = numChars * LANE_HEIGHT + TOP_BORDER;
    const overlayWidth = project.video.resolution?.[0] || 1920;
    canvas.width = overlayWidth;
    canvas.height = overlayHeight;

    // The playhead sits at 1/3 of the scrollable area (right of labels)
    ...
    try {
      ...
      const chunkDuration = 100; // 100 seconds per chunk = 15000 pixels
      ...
      for (let c = 0; c < numChunks; c++) {
        ...
        // Draw each character lane dialogues and markers for this chunk
        activeCharacters.forEach((char, i) => {
          const laneY = TOP_BORDER + i * LANE_HEIGHT;
          const blockH = LANE_HEIGHT - LANE_PADDING * 2;

          project.dialogues.filter(d => d.character_id === char.id).forEach(d => {
            // Absolute X position in the full timeline
            const absStartX = d.start_time * pps;
            const absEndX = d.end_time * pps;
            
            // X position relative to this chunk
            const startX = absStartX - (startTime * pps);
            const endX = absEndX - (startTime * pps);
            const blockWidth = endX - startX;

            if (endX < 0 || startX > chunkWidth || blockWidth <= 0) return;

            // Constrain to chunk bounds for drawing
            const clampedStart = Math.max(startX, 0);
            const clampedWidth = Math.min(endX, chunkWidth) - clampedStart;
            ...
            if (d.text && blockWidth > 20) {
              ...
              // Text shouldn't be squished if part of the block is outside the chunk,
              // but we only draw what's visible
              ctx.save();
              ctx.beginPath();
              // Clip to the exact part of the block that lies within the chunk
              ctx.rect(clampedStart, laneY + LANE_PADDING, clampedWidth, blockH);
              ctx.clip();
              // Start rendering at startX (even if it's negative, canvas will clip it)
              ctx.translate(startX + 8, laneY + LANE_HEIGHT / 2);
              // Deform based on the FULL block width, not clamped, so it flows continuously
        ...
        project.markers.forEach(m => {
          const mx = (m.time - startTime) * pps;
          if (mx >= 0 && mx <= chunkWidth) {
      ...
      // 2. Generate UI Canvas (Left panel, red line)
      canvas.width = overlayWidth;
      canvas.height = overlayHeight;
      ...
      // Label column background (opaque to hide tracks behind it during FFmpeg compositing)
  ...
  return (
    <div style={modalOverlayStyle}>
      <div style={modalStyle} className="glass-card">
        ...
        {!isExporting && !error && (
          <div>
            <p>Ajustez l'apparence de la bande avant l'encodage FFmpeg.</p>
            ...
            <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <div>
                ...
                <input type="range" min="0.5" max="2.0" step="0.1" value={scale} onChange={(e) => setScale(parseFloat(e.target.value))} style={{ width: '100%' }} />
              </div>
              ...
              <div>
                ...
                <input type="range" min="50" max="400" step="10" value={pps} onChange={(e) => setPps(parseInt(e.target.value))} style={{ width: '100%' }} />
              </div>
            </div>

            <div style={{ marginTop: '25px', display: 'flex', gap: '10px' }}>
              <button className="btn btn-primary" onClick={startExport}>Lancer l'Export final</button>
        ...
        {isExporting && !error && (
          <div>
            <p>{stage}</p>
            <div style={{ width: '100%', backgroundColor: 'rgba(255,255,255,0.1)', height: '10px', borderRadius: '5px', marginTop: '10px', overflow: 'hidden' }}>
              <div style={{ width: `${progress}%`, backgroundColor: '#4ade80', height: '100%', transition: 'width 0.2s' }} />
            </div>
          </div>
        )}
        ...
        {error && (
          <div>
            <h4 style={{ color: '#ef4444' }}>Export Failed</h4>
            <p style={{ color: '#f87171', fontSize: '12px', wordBreak: 'break-all' }}>{error}</p>
            <button className="btn btn-ghost" onClick={onClose} style={{ marginTop: '15px' }}>Close</button>
          </div>
        )}

        {/* Canvas is ALWAYS mounted so canvasRef is valid when startExport runs */}
        <canvas
          ref={canvasRef}
          style={{ display: 'none' }}
        />
